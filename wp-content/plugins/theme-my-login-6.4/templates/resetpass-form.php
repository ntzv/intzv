...
<div class="tml tml-resetpass" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	<p class="description indicator-hint">Это Ваш первый вход на сайт. Вам необходимо задать пароль для Вашей учётной записи</p>
	...
	<form name="resetpassform" id="resetpassform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'resetpass', 'login_post' ); ?>" method="post" autocomplete="off">
		...
		<p class="tml-submit-wrap">
			<input type="submit" name="wp-submit" id="wp-submit<?php $template->the_instance(); ?>" value="<?php esc_attr_e( 'Сохранить пароль', 'theme-my-login' ); ?>" />

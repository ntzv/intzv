 ...
 * @since 6.0
 */
define("DEFAULT_MIN_PASSWORD_LENGTH", 8); /*fix: установка минимальной длины пароля по умолчанию*/
...
class Theme_My_Login_Custom_Passwords extends Theme_My_Login_Abstract {
	...
	public function password_fields() {
		...
		<p class="tml-user-pass1-wrap">
			<label for="pass1<?php $template->the_instance(); ?>"><?php _e( 'Password', 'theme-my-login' );
				echo " ", sprintf( __( '(Must be at least %d characters.)', 'theme-my-login' ), apply_filters( 'tml_minimum_password_length', DEFAULT_MIN_PASSWORD_LENGTH ) ); /*fix:дополнительно отображать минимальную длину пароля*/?></label>
	...
	public function ms_password_fields() {
		...
		<span class="hint"><?php echo apply_filters( 'tml_password_hint', sprintf( __( '(Must be at least %d characters.)', 'theme-my-login' ), apply_filters( 'tml_minimum_password_length', DEFAULT_MIN_PASSWORD_LENGTH ) ) ); ?></span>

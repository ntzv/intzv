...
class WP_Custom_Post_Type_Widgets_Search extends WP_Widget {
	...
	public function add_form_input_post_type( $form ) {
		...
		//add: изменение плейсхолдера соответственно типу записей
		$placeholder = ! empty( $posttype ) ? get_post_type_object( $posttype )->labels->search_items : 'Искать везде';
		$form = str_replace( 'placeholder="Search …', 'placeholder="'. $placeholder, $form );

		return $form;
	}

	public function widget( $args, $instance ) {
		//add: для корректного отображения типа записи
		if ( ! isset( $args['widget_id'] ) ) {
			$args['widget_id'] = $this->id;
		}
	
		$posttype = ! empty( $instance['posttype']) ? $instance['posttype'] : 'post';
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? '' : $instance['title'], $instance, $this->id_base ); //fix: не показывать заголовок виджета

...
/*      if (isset($_REQUEST['column0']) && $_REQUEST['column0'] == "*") {
	  ...
	  if (isset($_REQUEST['column0']) && $_REQUEST['column0'] != "" && isset($_REQUEST['column1']) && $_REQUEST['column1'] != "") {
          ...
          while ($row = mysql_fetch_array($result, MYSQL_NUM)){
            $n = count($row);
            echo '<input type="radio" name="'.$column0.'_'.$column1.'" id="'.$column0.'_'.$column1.'_'.$row[0].'" value="'.$row[0].'" class="var_v_v"';
			if ($mark == $row[0]) echo ' checked';
			echo '> - <span class="var_v_v">';
            echo $row[1];
            echo '</span>&nbsp;&nbsp;&nbsp;&nbsp;';
          }
	      //echo '</select>';
        ...
        if (isset($_REQUEST['column0']) && $_REQUEST['column0'] != "" && $_REQUEST['column0'] != "*") {
          ...
          while ($row = mysql_fetch_array($result, MYSQL_NUM)){
            $n = count($row);
            echo '<input type="radio" name="'.$column0.'" id="'.$column0.'_'.$row[0].'" value="'.$row[0].'"';
			if ($mark == $row[0]) echo ' checked';
			echo '> - <span class="var_v_v">';
            echo $row[0];
            echo '</span>&nbsp;&nbsp;&nbsp;&nbsp;';
